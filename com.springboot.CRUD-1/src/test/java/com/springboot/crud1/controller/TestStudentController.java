...
//		assertEquals(200,status);
	...
	public void updateStudent() throws Exception
	{
		Integer studentId=1;
		ObjectMapper objectMapper=new ObjectMapper();
		String content=objectMapper.writeValueAsString(student1);
		
		when(studentService.updateStudent(student1, studentId)).thenReturn("true");
		
		MvcResult mvcResult= mockMvc.perform(MockMvcRequestBuilders.put("/students/{studentId}",studentId).contentType(MediaType.APPLICATION_JSON).content(content)).andExpect(status().isOk()).andReturn();
	    Integer status=mvcResult.getResponse().getStatus();
	...
	public void deleteByIdTest() throws Exception
	{
	    ...
	    MvcResult mvcResult=mockMvc.perform(MockMvcRequestBuilders.delete("/students/{studentId}",studentId)).andExpect(status().isOk()).andReturn();
