...
//        );
	...
	public void testUpdateById() throws Exception
	{
		...
		when(cloudService.updateById(cloudVendor,vendorId)).thenReturn("true");
	...
	{
		Integer vendorId=1;
		when(cloudService.deleteById(vendorId)).thenReturn("true");
		
	    MvcResult mvcResult=	mockMvc.perform(MockMvcRequestBuilders.delete("/cloudvendors/{vendorId}",vendorId)).andExpect(status().isOk()).andReturn();
